...
**2.2 确定关联论文（引用链 + 代码相关性双重筛选）**

关联论文的选取需要 **双重过滤**：先用引用链限定候选范围，再用代码相关性做最终筛选。

**第一层：引用链限定候选范围**

- **一级引用**：主论文 References 中直接引用的论文
- **二级引用**：一级引用论文的 References 中被引用的论文
- 引用链之外的论文不作为候选（不接受凭名称或关键词推测）

**第二层：代码相关性筛选**

对候选论文逐篇判断与代码的实际相关性：
- **高相关**：论文方法直接对应代码中实现的核心算法模块 → ✅ 下载
- **低相关**：论文仅在背景或对比中被引用，代码中没有对应实现 → ❌ 不下载
- **不确定**：无法明确判断是否有对应代码实现 → ❓ 询问用户
...
├── 一级引用 C ── 代码中无对应实现 → ❌ 跳过
│   └── 二级引用 C1 ── 代码中有对应实现 → ✅ 下载
├── 一级引用 D ── 不确定是否有对应实现 → ❓ 询问用户
└── 一级引用 E ── 仅背景引用，无代码对应 → ❌ 跳过
```

判定"代码中有对应实现"的依据：
- 代码中存在该论文描述的算法的独立实现（非调用第三方库）
- 代码注释中引用了该论文（`@see`、`// Ref:`、文件头注释等）
- 论文方法被代码中的关键数据流或处理流程使用

不作为收录对象的：
- 通用工具库依赖（Eigen、OpenCV、g2o、Ceres 等）
- 仅在主论文 Related Work 中被提及但代码未实现的论文
- 背景知识类引用（如综述论文）
...
2. **引用链 + 代码相关性双重筛选**：候选论文必须在主论文的一级/二级引用链内，且与代码有实际对应实现，两者缺一不下载
